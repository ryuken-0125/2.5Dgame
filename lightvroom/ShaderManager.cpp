    ...
    const std::wstring& shadowFilePath)
{
    ...
    // 1. PBR 頂点シェーダー
    hr = D3DCompileFromFile(pbrFilePath.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
    ...
    hr = D3DCompileFromFile(shadowFilePath.c_str(), nullptr, nullptr,
        "VSMain", "vs_5_0", compileFlags, 0, &vsBlob, &errorBlob);
    if (FAILED(hr)) { OutputErrorMessage(errorBlob.Get()); return false; }
    device->CreateVertexShader(vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(),
        nullptr, &m_shadowVertexShader);

    // 5. 影用 ピクセルシェーダー（透過切り抜き用）
    psBlob.Reset(); errorBlob.Reset();
    hr = D3DCompileFromFile(shadowFilePath.c_str(), nullptr, nullptr,
        "PSMain", "ps_5_0", compileFlags, 0, &psBlob, &errorBlob);
    if (FAILED(hr)) { OutputErrorMessage(errorBlob.Get()); return false; }
    device->CreatePixelShader(psBlob->GetBufferPointer(), psBlob->GetBufferSize(),
        nullptr, &m_shadowPixelShader);

    // 6. 画像貼り付け用サンプラー (s0 : WRAP)
    ...
    ID3D11ShaderResourceView* textureSRV)
{
    ...
    // テクスチャがあればスロット 0 にセット（シルエット切り抜き用）
    if (textureSRV)
    {
...
// パス2（本番描画用）
void ShaderManager::BindMainPass(ID3D11DeviceContext* context,
    ID3D11ShaderResourceView* shadowSRV)
{
    ...
    // t0=影テクスチャ、s0=画像用サンプラー、s1=影用サンプラー
